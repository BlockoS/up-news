...
int output_tilesets(tilemap_t* tilemap)
{
    ...
    for(i=0; ret && (i<tilemap->tileset_count); i++)
    {
        ...
        ret = png_load(tilemap->tilesets[i].filename, &img);
        if(!ret)
        {
        ...
        else
        {
            ...
            ret = image_to_tiles(&img, tilemap->tile_width, tilemap->tile_height, buffer, &size);
            if(ret)
            {
                ret = write_raw(filename, buffer, size);
                if(!ret)
                {
            ...
            if(ret)
            {
                ...
                ret = write_raw(filename, rgb_333, 32);
                if(!ret)
                {
                    fprintf(stderr, "failed to write %s\n", filename);
                }
    ...
    for(;;)
    {
        ...
        switch(c)
        {
            case 'v':
                ...
                if(errno || (vram_offset > 0xffff) || ('\0' != *endptr))
                {
